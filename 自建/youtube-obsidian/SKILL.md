...
# YouTube → Obsidian v1.4（阅读版：封面 + 分节总结一次展开 + 中文优先；课程类加均分截图，播客类仅封面）
...
> v1.4（2026-09-13：专名三条）：人名不音译、模型名不翻译、Agent 不译智能体（起因：v1.3 重写把 Lex 翻成莱克斯、DeepSeek Pro 翻成专业版——共识英文一律不动）。
...
- **专名三条（硬的，2026-09-13 v1.4）**：① 人名保留原文，不音译（Lex 不翻莱克斯）；② 模型名、产品代号保留原文，不翻译（DeepSeek Pro / DeepSeek Flash / Grok fast 不翻专业版、极速版）；③ Agent 不译成智能体（圈内共识英文，直接用 Agent，注意与汉字之间留空格）。起因都是"翻译过头"事故：共识英文一律不动，拿不准就保留英文。
